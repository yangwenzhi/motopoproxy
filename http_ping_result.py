...
def parse_result(response):
    ...
    for rec in j['hits']['hits']:
        try:
            value_obj = rec['_source']['value']['value_obj']
            ttl = value_obj['ttl']
            pingurl = value_obj['pingUrl']
            pingurl = pingurl.replace('/index.html','')
            if (ttl == 0):
                print(pingurl)
            if not pingurl in arr:
                arr[pingurl] = [0,0]
            if (ttl == 9999):
                arr[pingurl][0] +=1
            else:
    ...
    for k in arr.keys():
        #if (arr[k][1] == 0):
        print(k,arr[k][0],arr[k][1])
def main():
    resp = get_http_ping_result(rec_count=10000,begin_time='2022-12-01',end_time='2022-12-13',area='Iran')
    parse_result(resp)
    #print(resp)
